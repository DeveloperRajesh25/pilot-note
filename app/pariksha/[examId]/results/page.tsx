    ...
    <>
      ...
      <main className="grow pt-36 pb-24 bg-white">
        <div className="container mx-auto px-6 max-w-3xl">
          ...
          <h1 className="font-display text-4xl md:text-5xl text-neutral-900 leading-[1.05] tracking-[-0.03em] mb-3">
            {data.exam.title}
          </h1>
          ...
          <div className="border border-neutral-200 rounded-3xl p-10 md:p-16 text-center mb-10">
            <div className="relative w-44 h-44 mx-auto mb-6">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="font-display text-5xl text-neutral-900 leading-none tabular-nums">{pct}%</span>
                <span className="text-[10px] uppercase tracking-[0.22em] text-neutral-400 mt-2">{score}/{total}</span>
              </div>
            </div>

            <p className={`font-display text-3xl mb-1 ${tone}`}>
              {passed ? 'Pass' : 'Fail'}
            </p>
            <p className="text-neutral-500 text-sm mb-8">{verdictLine}</p>

            <div className="flex gap-3 justify-center flex-wrap">
              <Button variant="primary" href="/pariksha">All exams <ArrowRight className="w-4 h-4" /></Button>
              <Button variant="secondary" href="/profile">View profile</Button>
            </div>
          </div>
...
function Stat({ label, value, tone }: { label: string; value: string; tone?: 'emerald' | 'rose' }) {
  ...
  return (
    <div className="border border-neutral-200 rounded-2xl p-6">
      <p className="text-[10px] uppercase tracking-[0.22em] text-neutral-400 mb-2 font-medium">{label}</p>
