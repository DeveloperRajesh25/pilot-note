...
export async function POST(request: NextRequest) {
  ...
  const rows: { exam_id: string; question: string; options: string[]; correct: number; explanation: string | null }[] = [];
  const errors: string[] = [];
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    rows.push({ exam_id, question: trimmedQ, options: trimmedOpts, correct, explanation: explanation || null });
  }
  ...
  const { data, error } = await db.from('exam_questions').insert(rows).select();
